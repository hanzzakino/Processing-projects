...
void integrate_gx(float s, float xa, float xb){
  ...
  for(float x = xa; x <= xb && x+dx <= xb; x = x + dx){
      ...
      sum+=sqrt((abs((x-prevv)*scale_px)*abs((x-prevv)*scale_px)) + (abs((g(x)-g(prevv))*scale_px)*abs((g(x)-g(prevv))*scale_px)));
      prevv = x;
  }
  float last = sqrt( pow(abs((6-prevv)*scale_px),2) + pow(abs((g(prevv))*scale_px),2) );
  println((((sum+last)/scale_px)*4)/12);
}
